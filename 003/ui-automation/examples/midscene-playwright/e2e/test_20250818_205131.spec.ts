...
test("完整填写问卷并提交", async ({
  ...
  try {
    ...
    await aiWaitFor('问卷题目已显示', { timeoutMs: 10000 });
  } catch {
  ...
  await aiTap('下一题按钮');
  await aiWaitFor('网购频率问题已显示');

  // 5. 网购频率：选择"每周１次"
  await aiTap('频率选择中的"每周１次"选项');
  await aiTap('下一题按钮');
  await aiWaitFor('月均花费问题已显示');

  // 6. 月均花费：选择"３０１－５００元"
  await aiTap('花费选择中的"３０１－５００元"选项');
  await aiTap('下一题按钮');
  await aiWaitFor('促销方式多选题已显示');
  ...
  await aiTap('下一题按钮');
  await aiWaitFor('账户安全问题已显示');

  // 13. 网购最担心的是什么：选择"被盗刷账户"
  await aiTap('账户安全选择中的"被盗刷账户"选项');
  ...
  // 验证提交结果
  await aiWaitFor('提交成功或感谢参与提示已显示', { timeoutMs: 30000 });
  await aiAssert('页面显示"提交成功"或"感谢参与"等提示信息');
